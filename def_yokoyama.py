...
def dictinary_learning(D, dcon, xf, s, N, M, rho, d_size, ite=50):
    ...
    while True:
        ...
        for j in range(len(dd_conj)):
            ...
            for k in range(M+1):
                bf[k*N:(k+1)*N] = np.fft.fft(b[k*N:(k+1)*N])/np.sqrt(N)
...
# %%
exim = util.ExampleImages(scaled=True, zoom=0.1, gray=True)
...
for i in range(5):
    ...
    X, y, prx, dux, hize = coefficient_learning(d, X, y, S, N*N, M, rhox, lamd, d_size, ite= 500)
    prx_log.append(prx[0])
    dux_log.append(dux[0])
    hizero.append(hize[1])
    xf = X_to_xf(X, N*N, M)
    # d,df  = D_to_df_kai(D, N*N, M, d_size)
    # d = D_to_d(D, N*N, d_size)
    d, dcon, prd, dud = dictinary_learning(d, dcon, xf, S, N*N, M, rhod, d_size, ite = 500)
    prd_log.append(prd[0])
    dud_log.append(dud[0])
    d = d.reshape(M, N, N)
    d = d.transpose(1, 2, 0)
    d_Pcn = cnvrep.Pcn(d.reshape(N, N, 1, 1, M), (d_size, d_size, M), Nv=(N, N)).squeeze()
...
# %%
print("iterate number: ", roopcount)
imgr = sl1 + image*sh1[0][0]/image[0][0]
print("Reconstruction PSNR: %.2fdB\n" % sm.psnr(ori[0], imgr))
# %%
print(sh1)
# aaa = sh1.reshape(N*N)
...
plot.imview(ori[0], title='Original', fig=fig)
plot.subplot(1, 2, 2)
plot.imview(imgr, title='Reconstructed['+str(i+1)+"]", fig=fig)
# %%
# fig = plt.figure()
x_coef = np.arange(5)
x_dict = np.arange(5)
...
plt.title("x5 d10 rho0.01 lamd0.001: "+str(roopcount))
...
ax1 = fig.add_subplot(1, 2, 1)
ax2 = fig.add_subplot(1, 2, 2)
ax1.plot(x_coef[10:], coef_log[10:], marker="o", color = "red", linestyle = "--")
ax2.plot(x_dict[10:], dict_log[10:], marker="v", color = "blue", linestyle = ":")
# %%
fig = plt.figure()
ax1 = fig.add_subplot(1, 2, 1)
ax2 = fig.add_subplot(1, 2, 2)
